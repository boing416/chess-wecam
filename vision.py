...
def rank_moves(board, before, after):
    ...
    if total < 12 or max(signal.values()) < 6:
        return (
            [],
            changed,
            "Не вижу изменения позиции. Если вы уже походили, повторите снимок или укажите ход на доске.",
        )
    results = []
    ...
    if not results or results[0]["strength"] < 4 or results[0]["coverage"] < 0.42:
        return (
            results[:5],
            changed,
            "Не удалось уверенно определить ход по изменениям на доске. Повторите снимок или укажите начальное и конечное поле на доске.",
        )
