...
def decode_image(uri):
    if not isinstance(uri, str) or not uri.startswith('data:image/') or len(uri) > 12_000_000:
        raise ValueError('Нет корректного кадра с камеры.')
    image = cv2.imdecode(np.frombuffer(base64.b64decode(uri.split(',', 1)[1], validate=True), np.uint8), cv2.IMREAD_COLOR)
...
def rectify(image, corners):
    ...
    if not cv2.isContourConvex(contour) or abs(cv2.contourArea(contour)) < .025:
        raise ValueError('Углы должны образовывать четырёхугольник вокруг всей доски.')
    points *= [image.shape[1], image.shape[0]]
    matrix = cv2.getPerspectiveTransform(points, np.array([[0, 0], [SIZE-1, 0], [SIZE-1, SIZE-1], [0, SIZE-1]], np.float32))
...
def labelled(image):
    ...
    for r in range(8):
        for c in range(8):
            ...
            cv2.putText(out,label,(x,y),cv2.FONT_HERSHEY_SIMPLEX,.4,(0,0,0),3)
            cv2.putText(out,label,(x,y),cv2.FONT_HERSHEY_SIMPLEX,.4,(255,255,255),1)
    return out
...
def changed_squares(board, move):
    other = board.copy(); other.push(move)
...
def rank_moves(board, before, after):
    ...
    changed = [chess.square_name(s) for s,v in sorted(signal.items(),key=lambda x:-x[1]) if v > 6]
    if total < 12 or max(signal.values()) < 6:
        return [], changed, 'Не вижу изменения позиции. Сделайте ход и уберите руку из кадра.'
    results = []
    for move in board.legal_moves:
        ...
        results.append({'uci':move.uci(),'san':board.san(move),'score':round(score,3),'coverage':round(coverage,3),'strength':round(strength,2)})
    results.sort(key=lambda x:-x['score'])
    if not results or results[0]['strength'] < 4 or results[0]['coverage'] < .42:
        return results[:5], changed, 'Кадр неоднозначен: уберите руку, проверьте свет и неподвижность камеры.'
    return results[:5], changed, ''

def lm_status():
    try:
        with HTTP.open(LM_URL+'/models',timeout=2) as r:
            models = [m['id'] for m in json.load(r).get('data',[]) if 'embedding' not in m['id']]
        return {'available':bool(models),'models':models,'model':MODEL if MODEL in models else (models[0] if models else None)}
    except Exception:
...
def ask_model(prompt, images):
    ...
    if not status['available']:
        raise ValueError('LM Studio недоступен. Запустите локальный сервер и загрузите модель с поддержкой изображений.')
    content = [{'type':'text','text':prompt}] + [{'type':'image_url','image_url':{'url':encode(labelled(im))}} for im in images]
    data = {'model':status['model'],'messages':[{'role':'user','content':content}], 'temperature':0, 'max_tokens':1000, 'stream':False}
    req = urllib.request.Request(LM_URL+'/chat/completions',json.dumps(data).encode(),{'Content-Type':'application/json'})
    try:
        with HTTP.open(req,timeout=90) as r:
            answer = json.load(r)['choices'][0]['message']['content'] or ''
    except Exception as exc:
        raise ValueError('Модель не ответила на изображение. Проверьте загрузку vision-модели в LM Studio.') from exc
    answer = re.sub(r'<think>.*?</think>', '', answer, flags=re.S)
    match = re.search(r'\{[^{}]*\}',answer,re.S)
    if not match:
        raise ValueError('Модель не вернула понятный результат. Уточните позицию вручную.')
    return json.loads(match.group())

def recognize_move(board, before, after, use_model=True):
    ranked, changed, warning = rank_moves(board,before,after)
    result = {'candidates':ranked, 'changed':changed, 'warning':warning,'method':'Локальное сравнение кадров', 'uci':None}
    if not ranked or warning:
        return result
    best = ranked[0]
    if use_model:
        try:
            candidates = [m['uci'] for m in ranked if m['score'] > .15]
            answer = ask_model('Two photos of the SAME physical chessboard, BEFORE and AFTER one move. '
                'Each square is labelled. Current FEN before move: '+board.fen()+'. '
                ...
                'Return JSON only: {"move":"e2e4", "confidence":0.9} or {"move":null,"confidence":0}.', [before,after])
            uci = answer.get('move')
            confidence = float(answer.get('confidence',0))
            if uci in candidates and confidence >= .65:
                result.update(uci=uci,method='Gemma + сравнение кадров')
                return result
            result['warning'] = 'Модель не уверена в ходе. Выберите вариант или укажите ход на доске.'
            return result
        except ValueError as exc:
            result['warning'] = str(exc) + ' Показан результат сравнения кадров.'
    gap = best['score'] - (ranked[1]['score'] if len(ranked)>1 else 0)
    if best['score'] >= .55 and gap >= .12:
        result['uci'] = best['uci']
