...
class APITests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.proc=subprocess.Popen([sys.executable,'server.py','--port',str(cls.port)],cwd=Path(__file__).resolve().parents[1],env={**os.environ,'CHESS_CAM_DATA':cls.temp.name},stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
        cls.base=f'http://127.0.0.1:{cls.port}'
        for _ in range(100):
            try:
                with urllib.request.urlopen(cls.base+'/api/state',timeout=.2):return
    ...
    def post(self,path,**data):
        ...
        req=urllib.request.Request(self.base+'/api/'+path,json.dumps(data).encode(),{'Content-Type':'application/json'})
        with urllib.request.urlopen(req,timeout=10) as r:return json.load(r)
    def test_complete_camera_cycle_correction_and_undo(self):
        self.post('camera-reset')
        b=chess.Board();corners=[[0,0],[1,0],[1,1],[0,1]]
        ...
        self.post('cancel');self.post('propose',uci='e2e4')
        result=self.post('confirm');self.assertEqual(len(result['state']['history']),1)
        pending=result['state']['pending']['uci'];b.push_uci(pending)
        result=self.post('computer',image=encode(picture(b)))
        self.assertNotIn('verify_required',result);self.assertEqual(len(result['state']['history']),2)
        result=self.post('undo');self.assertEqual(result['state']['fen'],chess.STARTING_FEN);self.assertFalse(result['state']['synced'])
    def test_unmoved_computer_is_not_committed(self):
        b=chess.Board();self.post('calibrate',image=encode(picture(b)),corners=[[0,0],[1,0],[1,1],[0,1]])
        self.post('start',camera_mode=True,player='black')
        result=self.post('computer',image=encode(picture(b)))
        self.assertTrue(result['verify_required']);self.assertEqual(result['state']['history'],[])
    ...
    def test_invalid_and_stale_requests(self):
        with self.assertRaises(urllib.error.HTTPError):self.post('start',player='white',camera_mode=False,fen='bad')
        with self.assertRaises(urllib.error.HTTPError):self.post('start',player='white',camera_mode=False,revision=-1)
    def test_pieces_are_available(self):
        for piece in 'PNBRQKpnbrqk':
            with urllib.request.urlopen(self.base+'/pieces/'+piece+'.svg') as r:self.assertIn(b'<svg',r.read())
